...
char **str_wrd(char *str, char *d)
{
	...
	if (!d)
		d = " ";
	for (i = 0; str[i] != '\0'; i++)
		if (!the_delim(str[i], d) && (the_delim(str[i + 1], d) || !str[i + 1]))
			num_wrd++;
	...
	if (!s)
		return (NULL);
	for (i = 0, j = 0; j < num_wrd; j++)
	{
		while (the_delim(str[i], d))
			i++;
		k = 0;
		while (!the_delim(str[i + k], d) && str[i + k])
			k++;
...
/**
 * **str_wrd1 - splits a string into words
 * @str: the input string
 * @d: the delimeter
 * Return: a pointer to an array of strings, or NULL on failure
 */
