 ...
 * @command_path: a path for the current command
...
char *duplicate_string(char *source, int start, int stop);
char *find_command_path(CommandData *data, char *path_string, char *command);

/* Loop Functions */
int shell_loop(char **arguments);
...
char *copy_string_with_length(char *destination, char *source, int length);
...
StringList *create_list_node_at_end(StringList **head, const char *string,
		int number);
size_t count_strings_in_list(const StringList *head);
...
int substitute_command_string(char **string_array, char *substring);
